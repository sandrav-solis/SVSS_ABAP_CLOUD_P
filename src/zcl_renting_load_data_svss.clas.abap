CLASS zcl_renting_load_data_svss DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_RENTING_LOAD_DATA_SVSS IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.

    DATA: lt_brands    TYPE TABLE OF zrent_brands_SS,
          lt_cars      TYPE TABLE OF zrent_cars_SS,
          lt_customers TYPE TABLE OF zrent_cust_SS.
** BRANDS **
    lt_brands = VALUE #(
    ...
    DELETE FROM zrent_brands_SS.
    INSERT zrent_brands_SS FROM TABLE @lt_brands.
    SELECT * FROM zrent_brands_SS INTO TABLE
    @lt_brands.
    out->write( sy-dbcnt ).
...
** CARS**
    ...
    moneda = 'USD' alquilado = 'X' alq_desde =
    '20211106' alq_hasta = '20241206' )
    ).

    DELETE FROM zrent_cars_SS.
    INSERT zrent_cars_SS FROM TABLE @lt_cars.
    SELECT * FROM zrent_cars_SS INTO TABLE @lt_cars.
    out->write( sy-dbcnt ).
...
** CUSTOMERS**
    ...
    DELETE FROM zrent_cust_SS.
    INSERT zrent_cust_SS FROM TABLE @lt_customers.
    SELECT * FROM zrent_cust_SS INTO TABLE
    @lt_customers.
    out->write( sy-dbcnt ).
